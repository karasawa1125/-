...
<html>
  ...
  <body>
    ...
    <div class="sub_page">
      <h1>マイページ</h1>
      <ul>
        <li>お名前：<?php print($account["name"]); ?></li>
        <li>ID：<?php print($account["login_id"]); ?></li>
        <li>メールアドレス：<?php print($account["mail"]); ?></li>
      </ul>
      <form action="../controller/UpdateFormController.php">
      <button type="submit" name="account_id" value="<?php print($account["account_id"]); ?>">登録情報変更</button>
      </form>
      <form action="../controller/DeleteFormController.php">
      <button type="submit" name="account_id" value="<?php print($account["account_id"]); ?>">登録情報削除</button>
      </form>
      <a href="../">トップに戻る</a>
    </div>
    <script type="text/javascript">
